...
At the time of writing, \fn{smart\_read} can read files in N formats, listed in Table X.
...
Table X shows which value types are used, and the package used to support this.
