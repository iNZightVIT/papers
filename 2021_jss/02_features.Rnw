...
\pkg{iNZight} was originally designed as a tool for teaching statistics to students, but has found a dual-purpose in research groups as an in-house tool for rapid research development.\footnote{make nicer}
By making the interface simple, as shown in \cref{fig:inzight_main}, \pkg{iNZight} is very easy to learn initially and, unlike more complicated alternatives, \emph{re-learning} the software is equally simple, making it ideal for beginners and researchers who might infrequently need to perform analyses.
The intuitive interface is achieved by using familiar controls such as \emph{drag-and-drop}, \emph{drop-down} selection, and \emph{slider bars}.
