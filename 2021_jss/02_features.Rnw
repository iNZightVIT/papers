...
\begin{itemize}
    \item common type of data beginners may want to explore
    \item needs a time variable, either encoded correctly or can be coerced (or specified manually if known)
    \item time on the $x$ axis, responses typically connected by lines to see change over time; can be one or multiple series
    \item seasonal decomposition: trend + seasonal effect + residuals (STL algorithm)
    \item teaching-focus: animations to show decomposition
    \item Holt-Winters forecasts \cite{Holt_2004,Winters_1960}
    \item used in NZ high schools for time series module
\end{itemize}
...
The code used by \pkg{iNZight} uses \pkg{tidyverse} \citep{tidyverse} syntax, as this provides a simple introduction to \proglang{R} with many simpler and verb-like syntax for data wrangling, and is used the \emph{R for Data Science} \citep{Wickham_2017}. To demonstrate the script produced by \pkg{iNZight}, APPENDIX contains the script generated by the demonstrations in this section.
