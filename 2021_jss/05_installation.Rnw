...
Most of these are now on \gls{cran}, and work continues to publish the remainder.
...
Finally, Linux comes in many flavours, each with different collections and names of libraries.
...
The primary audience for \pkg{iNZight} is students new to statistics, and are unlikely to have the computer literacy required by other \glspl{gui} to install and run the software (including \proglang{R}).
...
This directory contains a \code{.Rprofile} file which automatically loads the \pkg{iNZight} package and launches the \gls{gui}.
...
The \pkg{iNZight} installer also includes an Update script which, when launched, triggers an update to the \proglang{R} packages.
...
It is not unusual for \glspl{gui} to perform package installation; \proglang{R} commander prompts the user to install several recommended packages on startup.
...
We have built a docker image for \pkg{iNZight}, allowing users on macOS and Linux to run the software without installing the system dependencies.
The downside of this approach is that the result is not as smooth as a native application, and also, as a \gls{gui}, requires a little more work from the user (particularly on macOS) to set up the necessary conditions for the app running in the container to project the \gls{gui} onto the host's screen.
More information can be found at \url{https://github.com/iNZightVIT/docker-inzight}.
...
Within the container, shiny is used to create the visual controls and perform reactivity events.
A users data is stored on the server temporarily, and is only accessible from that user's session: it cannot be shared or accessed by other users.
