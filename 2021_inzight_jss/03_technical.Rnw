...
\pkg{iNZight}'s interface is written with \proglang{R} using three main packages.
\pkg{gWidgets2} \citep{gWidgets2} provide a simple widget-based \gls{api} to building a cross-platform interface with \proglang{R}, with wrappers for Tcltk, Qt, and GTK2.
We choose GTK2, as it was the most feature rich and---at the time---had the best cross-platform support (see \cref{sec:os-reqs}).
The GTK2 binaries are access through \proglang{R} using the \pkg{RGtk2} package \citep{RGtk2}, and is translated from simple \pkg{gWidgets2} calls using the \pkg{gWidgest2RGtk2} package \citep{gWidgets2RGtk2}.
...
\Gls{oop} allows for \emph{inheritance}, so developers can describe a general class which can be shared to several related components, but which may have different layouts or methods;
...
The \class{iNZDataNameWidget} component visible in the top-left of \cref{fig:inzight_main_widgets} displays a list of documents the user can switch between.
...
For example, the \class{iNZFilterData} class contains a \fct{filter\_data} method which takes the user's input and passes it to an appropriate \emph{wrapper} functions.
...
@
A second advantage of having components calling external functions is that the wrapper functions can include the lower-level \proglang{R} code used to generate the result, which the \gls{gui} can fetch from the returned data and attach to the script described in \cref{sec:code-writing}.
...
ui$initializeGui()
\end{CodeInput}
Users now have access to the \class{iNZGUI} object to explore states and trigger actions for easier testing.
