...
Newcomers to statistics often need to learn both how to code using \proglang{R} whilst simultaneously learning the basic skills for data exploration, while many researchers need quick, easy methods to get new projects started.
...
The software is \emph{variable first}, meaning users do not need to first know or remember complicated statistical terminology to get the most from their data: the software provides a list of applicable methods given their current variable selection(s).

Similarly, data manipulation techniques such as filtering, renaming levels of factors, and even specifying survey designs, are all presented in simple step-by-step windows, many of which provide previews that help users to tweak the inputs to get the correct output.
...
Many new features and functionality are planned for the future of \pkg{iNZight}, the foremost being the ability to interact with more complex datasets, particularly those saved within a database, with as much processing done within the database as possible to speed up the interface for large datasets.
