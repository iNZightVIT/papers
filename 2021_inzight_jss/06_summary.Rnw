...
The software is \emph{variable first}, so users do not need to first know or remember complicated statistical terminology to get the most from their data: the software provides a list of applicable methods given their current variable selection(s), and effecitvely helps to guide them through the analysis.
