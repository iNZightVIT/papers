...
This paper introduces \pkg{iNZight}, a \gls{gui} system written in \proglang{R} through a collection of \proglang{R} pacakges, designed to bring data visualisation and analysis to a wider audience.
...
A key difference between \pkg{iNZight} and these other \proglang{R} \glspl{gui} is our much broader and less technically-skilled audience, ranging from education through to analysts and researchers in community groups and government agencies.
To engage such audiences, we have designed \pkg{iNZight} in such a was as to minimise demands for up-front knowledge (such as the names of test or procedures) and provide just-in-time information.
...
Further details on the design pricipals of \pkg{iNZight} are given in \cref{sec:why}.

% Designed with ``variable-first'' concepts, \pkg{iNZight} is more approachable that other similar interfaces, and has a focus on exploration, with a special emphasis on graphics.
...
\section{Why do we need another GUI?}\label{sec:why}
