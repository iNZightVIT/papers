...
To engage such audiences, we have designed \pkg{iNZight} in such a way as to minimise demands for up-front knowledge (such as the names of tests or procedures) and provide just-in-time information.\footnote{clarify what this is, or use better/simpler words}
...
The script can also be used as a stepping stone for learning to code in \proglang{R}, particularly useful for students and other users needing to reproduce results or automate workflows.\footnote{Perhaps this paragraph belongs later on.}
\end{leftbar}
...
The goal of democratisation is to make skills(?) accessible to a wide range of users who would otherwise be unable to access them.
The reasons for this restricted access vary, but often come down to resource availability, most prominently time and money, and in many cases, there remains a need for these skills despite resource shortages.
...
\begin{enumerate}
\item as much as possible should be automated using good defaults, so users may specify the minimum information required;
\item users should not need to know before hand the name of the thing they're trying to do---the software should display relevant options based on current choices, and guide users through complicated procedures; and
\item alternate options should be available when applicable.
\item \ldots anything else \ldots ?
\end{enumerate}

TODO: add some mention of ``rapid research development tool''
...
The barriers to access are significantly reduced, \ldots [end with something strong!]
