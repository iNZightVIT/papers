...
The basic mode of \pkg{iNZight} provides visualizations and analysis for rectangular data with variables in columns and observations in rows.
...
Each action invoked by the user runs some \proglang{R} code, which is appended to the \proglang{R} script, with some select instances of live-code display and editing available (more details on this in \cref{sec:code-writing}).
Originally the code-writing functinality was developed as a way for students to begin to learn how to code with \proglang{R} after already becoming familiar with the skills of data visualisation, exploration, and analysis:
...
However, the feature also makes \pkg{iNZight} a powerful research development tool by allowing users to quickly explore a dataset while simultaneously creating an \proglang{R} script that can form the basis of a robust, reproducible workflow in a research setting.
