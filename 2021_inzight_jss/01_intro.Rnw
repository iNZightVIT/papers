...
To engage such audiences and democratise data analysis skills, we have designed \pkg{iNZight} in such a way as to minimise demands for up-front knowledge (such as the names of tests or procedures) and provide just-in-time information.
...
In addition, the combination of \pkg{iNZight}'s simplicity and powerful toolset make it a popular choice for research organisations and national statistics offices, including the Australian Bureau of Statistics and StatsNZ.
...
This paper provides an overview of \pkg{iNZight}'s design principles, main features, technical details of its development, an introduction to the \emph{add-on} system, and a description of the install process.\footnote{QUESTION: Move/add to end of abstract?}


\section[How iNZight is democratising data]{Our design principles for democratising data}\label{sec:design-principles}


The goal of democratisation is to make data usable in accessible ways to a wider audience by removing barriers and restrictions to the access as well as analysis of data, and therefore the ability to apply the information contained within the data.
The reasons for barriers to access and analyse data often come down to the availability of time, funding and the required skills.
...
As a result, the national statistics offices in these countries find it difficult to recruit and retain people with good data science skills or afford to contract external expertise.
...
\pkg{iNZight} not only provides an accessible data analysis and graphics tool, but also does it without the need to upload data beyond their own data environment---%
possession and control over the data can be maintained.
...
And while the \proglang{R} \glspl{gui} mentioned in \cref{sec:intro} do this at some level, some key elements have been overlooked that iNZIght can now address.
...
With programming and most \gls{gui} systems, you basically cannot do anything until you know what you want to do and know/remember its name.
This is a significant barrier to getting started and also results in significant time-losses getting back up to speed after a period of inactivity and a loss of familiarity.
...
The basic mode of \pkg{iNZight} provides visualizations and analysis for data in standard units/cases-by-variables format.\footnote{better word/phrase here... ``tidy'' format?}
\pkg{iNZight} uses a ``variable-first'' design in which users drive the software by choosing variables rather than procedures.
...
Here, \emph{``Tell me about''} is really shorthand\footnote{??} for \emph{``Show and Tell''} because what is delivered instantly is graphics because we believe graphics are the most accessible information-rich artefacts for broad audiences, and also that people are less likely to do silly things when they look at their data first.\footnote{Can we find a citation for this?}

\begin{leftbar}
    \begin{itemize}
        \item add something about non-modifiable graphics
        \item avoiding issues of misleading graphs, as described by Alberto Cairo in ``How Charts Lie''
    \end{itemize}
\end{leftbar}
...
interactivity;
and also many additional modifications that might be desired for aesthetic reasons.

\begin{leftbar}
    \textbf{Anything in this para needs to be included in the following??}
...
Another feature of \pkg{iNZight} is its code-writing functionality, which it shares with other \glspl{gui} but executes in a different way.
Originally this was developed as a way for students to begin to learn how to code with \proglang{R} after aalready becoming familiar with the skills of data visualisation, exploration, and analysis.
However, the code-writing feature also makes \pkg{iNZight} a powerful research development tool by allowing users to quickly explore a dataset while simultaneously creating an \proglang{R} script that can form the basis of a robust, reproducible workflow in a research setting.
