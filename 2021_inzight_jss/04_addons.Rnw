...
The \pkg{iNZight} small area estimation module \cite{Elliott_Bryant_2021} provides a simple interface to several \proglang{R} packages that implement Bayesian demographic estimation: \pkg{dembase} \citep{dembase}, \pkg{demest} \citep{demest}, and \pkg{demlife} \citep{demlife}.
