...
In addition to the binaries and packages, the installer includes several Desktop shortcuts that to launch \proglang{R} in a specific directory.
...
The \pkg{shiny} package is used within the container to create visual controls and perform reactivity events.
