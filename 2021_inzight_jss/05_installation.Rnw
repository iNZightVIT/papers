...
However, the two main dependencies are \code{xorg} and \code{gtk}, which are typically installed using the system package manager.
...
Additionally, we include an Uninstaller which removes \pkg{iNZight} from the user's system if they so desire---since \pkg{iNZight} is standalone, this just deletes the folder and shortcuts.
...
There is a set of URL parameters which can be passed to the \pkg{iNZight} \emph{Lite} instance, including a URL for a dataset to automatically load, so for examples it's possible to store datasets on a server with a list of URLs to launch Lite with the chosen data loaded.
For example, \url{https://lite.docker.stat.auckland.ac.nz/?url=https://inzight.nz/testdata/nhanes.csv&land=visualize}.
An example is this dataset index, \url{https://www.stat.auckland.ac.nz/~wild/data/Rdatasets/}.
