...
It also hides the \proglang{R} console, so users are presented with just a \gls{gui}, as they will be most familiar with.
...
To provide these students with equal opportunity, we developed an online version of \pkg{iNZight} that uses \pkg{shiny} \citep{shiny} as the \gls{gui} framework instead of GTK, named \pkg{iNZight} \emph{Lite}.
...
There is a set of URL parameters which can be passed to the \pkg{iNZight} \emph{Lite} instance, including a URL for a dataset to automatically load, so for examples it is possible to store datasets on a server with a list of URLs to launch Lite with the chosen data loaded.
...
\textbf{Unhappy with this para either ...}
