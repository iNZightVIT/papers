...
To improve the accessibility of \pkg{iNZight}, we deploy an installer that is effectively a self-extracting archive that includes a copy of \proglang{R}, the package library, and GTK, so once installed, \pkg{iNZight} is ready to go.
