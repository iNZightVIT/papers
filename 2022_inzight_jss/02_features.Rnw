...
For example, for values $1,2,3$ instead of \mbox{``A'', ``B'', ``C''}, users can override the variable type from `numeric' to `categorical' and relabel the levels after import.
...
Subsequent inputs are then tailored to previous choices, thus guiding users through the procedure (\gls{dp}~4).
